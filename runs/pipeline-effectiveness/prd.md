...
**Status:** Draft v0.1
**Author:** John Pletka (drafted with Claude from a goals-first analysis of the pipeline, 2026-07-02)
**Date:** 2026-07-02
...
- **Q2 — Panel composition v1.** Two members (codex/gpt-5.5 + a claude-profile reviewer on lenses correctness/spec-coverage) or three (+ security)? A claude reviewer contends with the builder's window. *Proposal: start with two, both non-Anthropic if a capable second non-Anthropic profile exists; measure before adding a third.*
...
- **Q5 — Threshold ratification.** All §9 numbers (25% yield, 40% gate reduction, 50% re-litigation drop, ≤ 10% verifier cost) are proposals to react to, not measurements.
