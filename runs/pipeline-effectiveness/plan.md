...
- **Panel-member failure semantics — fail closed (authoritative human decision plan-cycle-resp-2(a); guiding principles "fail closed", "data over inference").** The ensemble review step defines explicit, fail-closed behavior for a panel member that errors or hits a usage limit, so a run never silently proceeds on a reduced panel or re-pays completed member calls on resume:
  - **Member error / usage-limit parks the cycle closed.** If any panel member's review step errors (adapter/parse failure, unexpected exit) or is parked by the transient usage-limit/overload classification (`adapters/failure_markers.py`, FR-3.1), the whole ensemble review step **parks fail-closed** — it never continues to dedup/triage with a silently reduced panel, and it never treats a missing member's findings as "clean." A usage-limit park is resumable (the parked run resumes when quota returns); a terminal member error escalates to a human. Either way the cycle does not advance on a partial panel.
  - **Completed-member artifacts persist and are reused on resume.** Each member's findings-schema artifact is persisted the moment that member completes (before the step as a whole finishes). On resume of a parked ensemble review, members whose persisted artifact already exists are **not re-invoked** — their findings are read back from disk and only the not-yet-completed members run. This is idempotent and content-addressed to the member's (profile, lens, review-scope) identity, so a resumed panel re-pays only the members that had not finished, never the completed ones.
  - **Window admission scales its estimate by panel size.** The pre-step window-admission estimate (`engine/ledger.py` window admission) for an ensemble review scales its projected usage by the number of configured panel members (reviewer/lens count), so a multi-member panel is admitted against a correctly-sized estimate rather than a single-reviewer one — an under-estimate that admitted a panel it cannot afford would defeat the fail-closed park.
...
- Panel-member failure semantics (authoritative human decision plan-cycle-resp-2(a)): a two-member panel where one member's review errors (stubbed adapter failure) **parks the ensemble step fail-closed** — dedup/triage never run on the surviving member alone, and the missing member is not treated as clean; a member parked by the usage-limit/overload classification parks the cycle resumably (asserted against the `failure_markers` classification, not a live limit). Resume reuse: an ensemble step resumed after one member persisted its artifact and the other did not re-invokes **only** the incomplete member — the completed member's findings are read from its persisted artifact and that member's adapter is not called again (asserted via a call-count/invocation spy on the completed member). Window admission: the pre-step window-admission estimate for an N-member panel scales by N (a two- and three-member panel produce proportionally larger estimates than a single-reviewer step), asserted against the `ledger` admission estimate.
- Any cycle-round-count fixture pins `max_rounds: 2` in-fixture (P9 coupling).

**Exit criteria:** tests green; a two-member panel run persists per-member artifacts, a deduped merged set, and per-member yield metrics readable from the manifest; `gauntlet doctor` fails on a misspelled/unavailable Gemini panel model id (review F-005); a panel-member error/usage-limit parks the ensemble step fail-closed (never proceeds on a reduced panel), a resumed ensemble step reuses completed members' persisted artifacts and re-invokes only the incomplete member, and window admission scales its estimate by panel size (authoritative human decision plan-cycle-resp-2(a)).
...
**Amendment note (2026-07-06 — Q3 re-resolved).** The originally-approved v1 sandbox backend (codex `workspace-write`) is not realizable on the pinned codex CLI: `codex exec` fires no PreToolUse hooks (BOOTSTRAP-NOTES #10 / `.gauntlet/pins.yaml`), so the §7 judge-enforced read-denial (items 1 & 4) is fail-open, and `codex sandbox` (which could OS-jail enumeration) requires an unprovisioned `[permissions]` profile on the installed CLI. Q3 is re-resolved to the alternative it already listed — **claude-code as the v1 verifier backend** — whose PreToolUse hooks make the judge-enforced confinement realizable as §7 requires. The tradeoff (hook-level vs OS-kernel enforcement, incl. the subprocess boundary) is stated in the enforcement boundary below and is deliberately accepted for v1. Codex remains a supported **post-v1** backend once a repo-managed `[permissions]` profile is provisioned and pinned (see Deferrals). Ratified by human operator 2026-07-06 with human-ratification-as-gate.
...
- `engine/verify.py`: an optional `verifier` sub-step between review and triage. An agent on a designated profile (Q3 re-resolved — see amendment note: **claude-code** backend on the disposable copy, with judge-hook-enforced read/write/network confinement) receives the phase's plan section (goal + acceptance clauses) and a **disposable copy** of the post-handoff worktree, executes the deliverable, and returns findings-schema output with `category: behavioral` and the executed commands as evidence.
...
- **Migrate `acceptance_gate` collector enumeration into the sandbox backend (review F-002):** now that the P5 verifier sandbox exists, move `acceptance_gate`'s collector enumeration (P2's `pytest --collect-only`, which executes branch-authored `conftest`/test code at import) off the P2–P4 interim judge-hooked-subprocess posture and run it *inside* the same sandbox backend the verifier uses (read-confined to the run worktree copy, network default-deny, resource-bounded), keeping the fail-closed park-on-failure semantics. A migration test proves enumeration executes within the sandbox jail on the v1 backend, not in the interim path. Under the claude-code v1 backend this runs as a judge-hooked `Bash` `pytest --collect-only` call, read-confined to the copy by the hook and subject to the subprocess-boundary note above; the fail-closed park-on-failure semantics are unchanged.
- `.gauntlet/config.yaml` / `pipelines/standard.yaml`: verifier profile config.

**Sandbox backend + enforcement boundary (v1) — amended 2026-07-06 (Q3 re-resolution).** The contract items above are enforced by a single named backend, not by ad-hoc per-item hacks:
- **Backend:** the claude-code CLI launched with `--setting-sources project` so the engine-managed judge PreToolUse hook fires on every tool call (verified on pinned claude 2.1.190, `.gauntlet/pins.yaml`). The verifier runs with a `permission_mode` and an explicit `allowed_tools` allowlist; the disposable copy is its working directory. **Enforcement is hook-mediated (tool-call level), not OS-kernel-level** — the deliberate, ratified v1 tradeoff: it makes the §7 read-denial contract *enforceable at all* (the codex backend could not fire it), at the cost that confinement is applied per agent tool call rather than by a kernel jail. The engine refuses to run the verifier unless the judge hook is confirmed firing.
- **Filesystem read/write denial (outside the copy):** the judge session that hooks the verifier denies any tool call (Read/Write/Edit, and `Bash` commands) whose resolved path escapes the disposable-copy root (symlink-resolved, `..`-normalized). The copy is the only path the verifier may read or write. Outside-copy reads are *denied*, not merely read-only.
- **Network default-deny:** network-capable tools are withheld from the verifier's `allowed_tools` (no WebFetch/WebSearch), and the judge hook denies `Bash` commands that reach the network (default-deny on known network invocations); there is no per-verifier allowlist in v1 (network-permitted profiles are deferred below).
- **Env stripping:** the verifier process is spawned from a *rebuilt* environment — an explicit allowlist of variables (PATH, HOME, LANG, TERM, runtime essentials) copied forward; everything else, and specifically `*_TOKEN`, `*_KEY`, `ANTHROPIC_*`, and known cloud-credential vars, is absent from the child env (strip-by-construction, not deny-by-pattern-at-read). This is the primary defense that makes the hook-level boundary acceptable: even a subprocess the hook does not see has no credentials to exfiltrate and nothing outside the copy worth reaching.
- **Subprocess boundary (honest limit):** children spawned by a `Bash` command the verifier runs are *not* independently gated by the PreToolUse hook (the hook sees the top-level command, not its forks) and are *not* OS-jailed under this backend. Subprocess confinement rests on the stripped env (no creds reachable) and the disposable, hash-guarded copy (no durable effect on the real tree). A run that requires kernel-level subprocess isolation must use the deferred codex `[permissions]` backend, not v1.
- **Resource/wall-clock limit:** a wall-clock deadline (config, default bounded) and the platform's process resource limits are applied to the verifier's process group; expiry kills the group and fails the sub-step closed.
- **Unsupported-host detection (fail closed):** at sub-step start the engine probes that the claude-code backend is present and its PreToolUse judge hook actually fires (under `--setting-sources project`). If the hook cannot be confirmed firing, the sub-step **parks closed** — it never falls back to running the verifier unhooked/unsandboxed. This is the only supported v1 posture; other isolation backends are out of scope for v1.
...
- Sandbox tests (integration where a real sandbox is required): an outside-copy credential-file read is denied; a network reach under default-deny fails; a stripped secret env var is absent from the verifier process env; an over-limit execution is killed and parks (never "skipped, proceed"); a stubbed backend-probe failure parks the sub-step closed (verifier never runs unsandboxed); the backend probe confirms the judge hook fires, and a stubbed non-firing hook parks the sub-step closed.
...
**Deferrals:** network-permitted verifier profiles beyond the default-deny posture → per-phase acceptance-clause-gated config, post-v1. The `auto_when_clean` consumption of the verifier-clean signal → P8. Codex `workspace-write` + repo-managed `[permissions]` verifier backend (OS-kernel isolation, incl. kernel-level subprocess/network confinement) → post-v1, gated on provisioning and pinning the `[permissions]` profile.
...
- **FR-6/FR-4 interaction (authoritative human decision plan-cycle-resp-2(b); PRD §8):** a cycle whose **final round holds a carried/open remainder** fails the P8 `auto_when_clean` clean predicate and the gate **parks** — the carried remainder makes the round non-converged with an open `fix_now`-derived finding, so the strict clean conjunction (round-1 convergence · zero open blocking/major legitimate findings) cannot hold, and the gate parks for a human rather than auto-approving. The fixture drives a cycle to its final round with a remainder still carried, asserts the clean predicate is False citing the remainder, and asserts the gate parks (no `auto_approval` record written). This proves the convergence-carry (FR-6) and the evidence-tiered gate (FR-4) interact fail-closed on the last round, closing the gap where a carried remainder could otherwise be auto-approved away.
...
**Exit criteria:** tests green including the two issue-#49 escape fixtures caught 100% deterministically; the FR-6/FR-4 interaction fixture shows a final-round carried remainder failing the `auto_when_clean` clean predicate and parking the gate (authoritative human decision plan-cycle-resp-2(b)); the `max_rounds: 3` re-run of P1/P5/P8 cycle/gate suites passes; average rounds-per-cycle metric available for the §9 honesty-tax check.
...
- id: P1
  ...
  acceptance:
    ...
    - id: P1-A6
      clause: A panel member's error or usage-limit park fails the ensemble review step closed — the cycle parks and never proceeds to dedup/triage on a silently reduced panel (the missing member is not treated as clean).
    - id: P1-A7
      clause: On resume of a parked ensemble review, members whose findings artifact already persisted are not re-invoked — their findings are read back from disk and only the not-yet-completed members run (completed member calls are not re-paid).
    - id: P1-A8
      clause: The pre-step window-admission estimate for an ensemble review scales its projected usage by the configured panel size (reviewer/lens count), not a single-reviewer estimate.
- id: P2
...
- id: P9
  ...
  acceptance:
    ...
    - id: P9-A6
      clause: A cycle whose final round holds a carried/open remainder fails the P8 auto_when_clean clean predicate and the gate parks (no auto_approval record) — the FR-6/FR-4 interaction, proving a carried remainder is never auto-approved away on the final round.
```