...
- **FR-10.4** Bind `127.0.0.1` only (refuse non-loopback hosts, mirroring the judge). Require a per-serve token (constant-time compared, env-overridable `GAUNTLET_WEB_TOKEN`, distinct from the judge token), set as an httpOnly cookie so SSE/HTMX carry it. (The judge reads its token from an `X-Gauntlet-Token` header, but a browser cannot attach custom headers to SSE streams or native form POSTs, so the console uses a cookie — which is precisely why state-changing POSTs also carry a CSRF token, FR-10.6, that the header-based judge does not need.) `/healthz` is the only unauthenticated route.
...
- **Loopback-only bind + token auth**, mirroring the judge service's loopback + constant-time-token model (token printed on startup; env override). Token *delivery* differs by client: the judge reads an `X-Gauntlet-Token` header from its programmatic caller, whereas the browser-facing console carries the token in an httpOnly cookie for SSE/HTMX — which is why POST forms also carry a CSRF token (a header scheme would not need one). `/healthz` unauthenticated; everything else gated.
