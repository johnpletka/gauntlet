...
│  Jinja-rendered HTML + shim  ◀── SSE (live state + log tail) ──  in-tab notifications  │
└───────────────────────────────────────┬───────────────────────────────────────────────┘
...
- **D5 — Server-rendered (Jinja + a vendored vanilla-JS shim), not a SPA.** `jinja2` is already installed transitively (fastapi/litellm pull it) and the console makes it an **explicit** dependency (FR-11.3); live updates ride a ~30-line vendored vanilla-JS shim (`static/live.js`) over SSE — semantically equivalent to HTMX-over-SSE (the SSE stream carries only *signals*; the authoritative HTML always comes from a partial re-fetch) but with **zero fetched dependency**. The data is document-shaped and server-authoritative; partial HTML swaps over SSE match it without a build toolchain — consistent with the thin-by-design ethos. *(HTMX was the originally-approved mechanism; the shim substitution was discovered in P2 review (F-003) and **ratified 2026-06-17** rather than reverted — see the §11 DEV-1 deviation record.)*
...
- **FR-11.3** Minimal new dependency footprint. FastAPI + uvicorn are already **direct** dependencies; `httpx` and `jinja2` are currently only **transitive** (pulled in by fastapi/litellm today), so the console **promotes them to explicit `pyproject.toml` dependencies** rather than relying on a transitive graph that a future version bump could silently drop. Live updates are delivered by a ~30-line vendored vanilla-JS shim (`static/live.js`) over SSE — **no HTMX, no fetched dependency** (substitution ratified, §11 DEV-1); no `watchdog`, no `websockets`, no frontend build toolchain. Net: two already-installed deps made explicit, zero new heavy deps.
...
### Ratified deviations (post-approval)

Deviations from the approved PRD discovered during implementation and **ratified by the human operator** (per CLAUDE.md §2 "humans ratify; agents propose") rather than reverted. Recorded here as the audit trail.

- **DEV-1 [ratified 2026-06-17 — live-update shim in place of HTMX].** P2 shipped a ~30-line vendored vanilla-JS shim (`src/gauntlet/web/static/live.js`) for live row/log updates instead of the originally-approved vendored HTMX asset (the builder could not fetch the library). P2 review surfaced this as F-003 (a material change to an approved dependency/UI architecture). **Decision: ratify the shim, do not vendor HTMX.** Rationale: the shim is *functionally equivalent* to HTMX-over-SSE — the SSE stream carries only signals (`snapshot`/`transition`); the authoritative HTML is always re-fetched as a server-rendered partial, so a live swap equals a reload — and it adds **zero** fetched dependency, which is *more* faithful to the thin-by-design / no-build-step ethos (D5, FR-11.3, M5) than vendoring an external library. FRs/decisions updated to match: D5, FR-11.3, P2 (§8), and the §4 architecture diagram. No functional requirement is weakened; the live-freshness contract (FR-8.3) is unchanged.
