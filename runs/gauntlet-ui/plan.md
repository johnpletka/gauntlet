...
- **Exactly one sanctioned engine phase (D7), containing exactly two explicitly approved engine modifications, both landed in P3 and nowhere else:**
    ...
    2. the repo/worktree-scoped active-run lock (FR-10.5) — touches **only** `RunManager._refuse_if_active_run()` (`src/gauntlet/engine/run.py:260`) and its acquire/release call sites in the three driving verbs `RunManager.start()` / `resume()` / `approve()` (FR-10.5 requires all three to fail closed, not just `start`). All within `run.py` + `cli.py`; no third engine seam.
...
- **Stack / deps (FR-11.3):** FastAPI + uvicorn (already direct). `httpx` and `jinja2` promoted from transitive to **explicit** `pyproject.toml` deps in P1 (the first phase that imports them). Live updates ride a ~30-line vendored vanilla-JS shim (`static/live.js`) over SSE — **no HTMX, no fetched dependency** (substitution ratified post-approval, prd.md §11 DEV-1 / F-003). No `watchdog`, no `websockets`, no frontend build step. Net new heavy deps: zero (M5).
...
- **Reused engine facts the plan commits to** (verified against the tree, so a phase's assumption is grounded, not hoped):
    ...
    - `engine/cycle.py` writes the escalation `notes` that FR-4.6/FR-5.2 (EXP-1) key on when a cycle parks: `"escalation: …(FR-10.4 upstream invalidation): F-…"`, `"escalation: legitimate blocking finding(s) not fixed this round (FR-10.5): …"`, `"escalation (FR-10.5): max_rounds=…"`, and `"escalation (review F-009): …"` — all on a `StepRecord` of `type == adversarial_cycle`, `status == parked`. The classifier reads these as a closed, engine-grounded vocabulary (table-tested, R3); no manifest field is added.
...
- `GET /events` — SSE stream of run-list-level transitions; live row updates on the run-list and run-detail pages driven by a vendored ~30-line vanilla-JS shim (`static/live.js`) that re-fetches server-rendered partials on each signal (no HTMX — substitution ratified, prd.md §11 DEV-1 / F-003). Auto-reconnect re-reads current state on connect.
...
**Assumption validated:** the UI can resolve a parked gate's `show:` artifacts (and a parked `adversarial_cycle`'s escalation evidence), render a deterministic phase diff, and drive approve/reject/resume via **sanctioned verbs** without bypassing any invariant — the human-value core (G2/G3/G5).
...
- `resume_intel(manifest) → {state, recommended_action, rationale, available_controls}` (FR-5.1) — a **pure** classifier keyed on the **existing `StepRecord.status` enum**, using `notes` substrings *only* to split the two `halted` sub-cases (timeout vs budget). The recognized note vocabulary is **normative and grounded in what the engine actually writes** (not builder-invented), so a wording drift is caught by a test rather than silently mis-classifying:
    ...
    - **escalation sub-kind (FR-4.6/FR-5.2, EXP-1)** — when the current step is a parked `adversarial_cycle` whose `notes` begin with the case-insensitive marker **`"escalation"`** (engine emits these in `engine/cycle.py`: `"escalation: …(FR-10.4 upstream invalidation): F-…"`, `"escalation: legitimate blocking finding(s) not fixed this round (FR-10.5): …"`, `"escalation (FR-10.5): max_rounds=…"`, `"escalation (review F-009): …"`), the sub-kind is split by substring: `"upstream invalidation"`/`"FR-10.4"` → upstream-artifact reconciliation; `"FR-10.5"`/`"max_rounds"`/`"not fixed this round"` → exhausted-rounds / open-blocker; `"review F-009"` → unresolved blocker. `recommended_action` = "reconcile the named upstream artifact (or apply the in-code fix), **then** resume — a bare resume re-runs the cycle and re-parks"; `available_controls` = Resume only (no approve/reject — the engine offers neither for a cycle park).
    - These are the **only** two places `notes` text is interpreted (`halted` halt-kind and `parked`-`adversarial_cycle` escalation-kind); every other field comes from the typed status enum. The structured `halt_kind`/`gate_kind` enum that would remove this string match entirely is the named v1→v2 deferral below.
  Implements the full FR-5.2 mapping (gate→Approve/Reject; escalation-park→Reconcile-then-Resume; interrupted→Resume; timeout-halt→"resume re-triggers; raise timeout first" guidance; budget-halt→"raise budget_usd first" guidance; ambiguous/unknown halt→generic "inspect log/diff; raise the relevant guard" guidance; failed→"resume won't help"+log/diff; `rejected:`→terminal). Never offers a meaningless control (FR-5.3).
- `GET /api/runs/{slug}/gate[?run_id=]` — resolves the gate's `show:` list (read from the run's snapshot `pipeline.yaml`), each name resolved first against `run_dir/artifacts/<name>` then the slug-dir artifact root (FR-4.2). Detect gate via `current_step` → step with `status==parked` and `type==human_gate` (FR-4.1). **For a parked `adversarial_cycle` (escalation, FR-4.6):** the same endpoint resolves the cycle's `artifacts/findings.json` + `artifacts/triage.json`, parses the escalated finding id(s) from the step `notes`, and returns the escalated findings + their triage verdicts (claim/evidence/suggested_fix/reasoning/target_artifact) framed as a reconciliation decision — **never** approve/reject controls.
...
- UI: gate-review panel (findings/triage/confirm as readable tables; markdown artifacts rendered; phase-diff view), failure/resume panel driven by `resume_intel`, **escalation-park reconciliation panel** (FR-4.6: escalated findings + triage verdicts + named upstream artifact + phase diff, "reconcile then resume" banner, Resume-only control), the **optional opt-in scoped-analysis hand-off** (FR-4.7: assembles a copy-pasteable read-only prompt + `claude` invocation; the console spawns nothing and makes no model call), `judge-audit.jsonl` viewer (FR-3.4), upstream-conflict surfacing (FR-4.5). **Destructive-verb confirm step** for abort/reject (FR-10.7) — a UX-safety confirmation token, not a security control.
...
- `resume_intel`: **table-driven** over one fixture manifest per FR-5.2 row (keyed on the status enum), so a note-wording change fails a test rather than silently mis-classifying (R3). The table includes the two normative `halted` sub-cases (`notes` containing `"timeout halt"` → timeout guidance; `"budget halt"` → budget guidance) **and** the fail-closed cases: a `halted` note containing **both** markers and a `halted` note containing **neither** both classify to the generic "inspect log/diff; raise the relevant guard" recommendation (review F-003). It **also** covers the three `parked`-`adversarial_cycle` escalation sub-kinds (one fixture each: `"FR-10.4 upstream invalidation"`, `"FR-10.5 … max_rounds"`, `"review F-009"`) → each maps to Reconcile-then-Resume with Resume-only controls and the named reconciliation; a fixture asserts approve/reject is **never** offered for a cycle park.
- Escalation surface (FR-4.6): a fixture run parked at an `adversarial_cycle` with an `escalation: …F-003` note + real `artifacts/findings.json`/`triage.json`; assert the endpoint parses the escalated id(s), returns the finding+triage evidence, names the `target_artifact`, and exposes Resume-only (no approve/reject). Assert the FR-4.7 hand-off, when enabled, returns an assembled prompt string and **launches no subprocess / makes no model call** (the console stays control = CLI verbs).
...
**Deliverables**
- `web/notify.py` wired to the P2 watcher event bus. Fires on four transition *kinds* — **gate-reached** (parked at a `human_gate`, distinct from a halt), **escalation-parked** (parked at an `adversarial_cycle` whose `notes` begin with `escalation` — FR-4.6, EXP-1), **run-failed**, **run-completed** — de-duplicated **per `(run_id, transition-kind, current_step)`** (FR-9.1) so each distinct decision point notifies once.
...
**Test strategy**
- Inject transitions with a stub notifier; assert **one** fan-out per `(run_id, transition-kind, current_step)` — including a fixture parking at **two** gates in different steps → **two** gate notifications; **and a fixture parking at an `adversarial_cycle` escalation → one escalation-parked notification** (distinct kind, not collapsed with gate-reached); assert an unchanged-manifest re-poll → **zero**; assert a *raising* notifier is swallowed (run unaffected); assert Slack call shape via a mocked `httpx` transport.

**Exit criteria:** green; gate/escalation-park/fail/complete fire to configured channels, fail-soft, edge-triggered (M1).
