...
- **Orchestrator-owned response checkpoint commit (FR-2.2):** a new
  ...
  pending` / `… consumed`. The helper is **idempotent**: if the staged
  bookkeeping paths are byte-identical to what HEAD already commits (nothing to
  stage), it is a no-op and creates no empty commit; otherwise it commits. This
  is what lets crash recovery call it to *flush a not-yet-committed state*
  safely whether or not that commit already landed.
  ...
  - **Crash reconciliation (commit lands before manifest bookkeeping, and vice
    ...
    manifest (already showing the new `state`) and **flushes the matching
    checkpoint commit for that exact state via the idempotent helper before doing
    anything else** (see the recovery rule below); die after the commit → the
    state is already in git and the flush is a no-op. This reuses the same
    "branch tip slightly ahead of the manifest's last recorded commit is fine"
    reconciliation `resume()` already encodes (`run.py:806-821`) rather than
    inventing a second rule.
  - **Pending state must reach git *before* re-launch (F-002):** the dangerous
    case is a crash after the pending `write_atomic` but before its checkpoint
    commit. If recovery simply re-launched the existing pending entry, the *next*
    checkpoint commit would be the `consumed` one (written only after
    re-execution finishes) — so the first git-visible commit carrying the
    response would record `consumed`, collapsing the required `pending` and
    `consumed` git-visible states into one. The fix: on recovery of a
    still-`pending` entry the orchestrator **idempotently flushes its `pending`
    checkpoint commit first** (`_commit_manifest_checkpoint("… pending")`, a
    no-op if it already landed) and only **then** launches the adapter — so a
    distinct `pending` commit always precedes the later `consumed` commit, no
    matter where the original crash fell.
...
- **Append protocol (FR-2, FR-2.2, FR-7.1):**
  - If latest `human_responses` entry is `state=="pending"` (crash recovery): do **not** append; a **different** `--response` errors with the exact FR-7.1 message; an identical `--response` (or none) **first idempotently flushes the `pending` checkpoint commit** (F-002, above) and **then** re-launches the existing pending entry, reusing its `response_id`.
...
- **Re-execution + atomic finalize-and-consume (FR-2.2, FR-6, FR-7.1):** drive
  re-executes the parked agent_task (existing `_execute` path; `resuming` stays
  false for a clean PARKED re-entry). The terminal `_finalize` performs the
  consume **in the same single `write_atomic`** that records the step status and
  (only on `FAILED`) increments `attempts`: it transitions the latest pending
  entry to `state="consumed"`, stamps its finalized outcome on the entry,
  persists once, then calls `_commit_manifest_checkpoint`. Folding the consume
  and the attempt-increment into one atomic on-disk transaction is the
  **deduplication boundary** (F-003): there is no intermediate state in which an
  entry is finalized-but-still-`pending`, so a crash lands either *before* the
  write (entry stays `pending`, `attempts` not yet advanced → a clean re-launch
  counts the failure exactly once) or *after* it (entry `consumed` with its
  outcome recorded, `attempts` advanced once → recovery never re-executes it).
  The orchestrator remains the sole committer; the builder gets no direct-write
  path.
- **Cross-state crash recovery (FR-7.1, the full append→consume window):** the
  ...
  recovery check in `RunManager.resume` keys on the latest entry's **`state`**
  and runs for `PARKED`, `RUNNING`, and `INTERRUPTED` alike:
  - latest entry `pending` → flush its `pending` checkpoint commit (F-002), then
    re-launch the existing entry, do **not** append, reuse its `response_id`.
  - latest entry `consumed` → the atomic finalize-and-consume transaction already
    completed, so recovery performs **bookkeeping only**: idempotently flush the
    `consumed` checkpoint commit if it did not land, and **never** re-execute or
    re-increment (F-003).
  Because consume and the `FAILED` increment share one atomic write (above), a
  crash never yields a finalized-but-`pending` entry, so this `state`-keyed
  recovery re-enters with the same single entry and counts each logical failure
  exactly once across the whole window — no duplicate, no double attempt-count.
...
- **Retry-budget decoupling (FR-6):** FR-6 redefines `StepRecord.attempts` as the
  **failure-retry counter — it increments only when a run ends in failure**, never
  ...
  - **Persistence across restarts (the dedup boundary):** for an ordinary
    (non-response) step the increment is immediately followed by the existing
    write-ahead `_persist` (`:251`), so the failure count is durable before control
    returns. For a **response** resume, the increment, the pending→`consumed`
    transition, and the persist are a **single atomic `write_atomic`** (see
    *Re-execution + atomic finalize-and-consume*), so a crash after a `FAILED`
    finalize finds the entry already `consumed`; recovery then reconciles only the
    checkpoint commit and **does not re-execute** (`state`-keyed recovery, above),
    so one logical response failure increments `attempts` exactly once, never twice
    (F-003).
...
- Idempotency / fault injection across the **full** append→consume window — kill
  ...
  - after the pending `write_atomic` but **before** the `pending` checkpoint
    commit (F-002) → on recovery, assert a checkpoint commit showing `pending`
    reaches git history **before** the adapter is re-launched and **before** any
    `consumed` commit, so the two states never collapse into one.
  - after the pending append **and** its `pending` commit (record still `PARKED`).
  - after the write-ahead `RUNNING` checkpoint (`:234`) but before the adapter returns.
  - during adapter execution.
  - after the atomic finalize-and-consume `write_atomic` but **before** its
    checkpoint commit (entry already reads `consumed` on disk) → recovery flushes
    only the commit, performs **no** re-execution, and leaves `attempts`
    unchanged (F-003).
...
- **FR-6 (per-outcome attempt accounting), each driven independently:**
  ...
  - crash after a `FAILED` response finalize (entry now `consumed` on disk), then resume → recovery reconciles only the checkpoint commit, does **not** re-execute, and `attempts` reflects a **single** increment for that failure, not two (F-003).
...
- id: P3
  ...
  goal: Add --response, record entries idempotently (pending→consumed) with atomic persist plus a new idempotent orchestrator-owned manifest-checkpoint commit (_persist does not commit today) so both states reach git history, surviving kill -9 across the full append→launch→consume window (PARKED/RUNNING/INTERRUPTED, incl. dirty-base recovery). On recovery of a pending entry, idempotently flush its pending checkpoint commit before re-launch so pending always precedes consumed in git history (F-002); fold the pending→consumed consume and the FAILED attempt-increment into one atomic finalize transaction, with state-keyed recovery (consumed → bookkeeping-only, no re-execute) so a crash never re-executes or double-counts an already-finalized failure (F-003). Relocate StepRecord.attempts to a failure-only increment so conflict/response resumes never consume the failure-retry budget while a genuine response failure still counts once (FR-1/FR-2/FR-6/FR-7.1). Validates the determinism and crash-recovery core with fake adapters.
- id: P4
