...
**Deferrals:** auto-resume scheduling → P2. `halt_reason` enum + full status fields → P3. Cycle sub-step checkpointing (a killed *cycle* still loses its round) → P5.
...
- `engine/config.py`: `keep_awake: bool` (default false), `suspend_credit_cap_s`, heartbeat interval knobs. `keep_awake: true` wraps the driver in `caffeinate -i` on darwin; non-darwin ignores with a warning.
...
- Unit: three synthetic states → `host_suspended` / `driver_orphaned` / `agent_silent` classifications in `status --json`.
...
- `.gauntlet/config.yaml` + `pipelines/standard.yaml`: mechanical steps — `message_agent` (commit-message) and resume-disposition emission — bound to a designated cheap profile (e.g. `mechanic: {adapter: api, model: gpt-5-mini}`), not the builder profile. (Profile *assignment* only; the cheap profile's effort *value* is Q2.)
...
**Test strategy:**
- Unit (instrumented stub): overlapping in-flight triage calls; all-success round → final `triage.json` identical to a sequential run of the same fixtures.
...
- Unit (judge): two byte-identical calls → one evaluation + one audited hit; a denied call repeated → two evaluations; a policy edit between identical calls → two evaluations; a stubbed classifier is **not** invoked on the second identical allow (fast-path <150ms).

**Exit criteria:** triage wall-clock for a 5-finding round ≤40% of the sequential baseline; judge LLM-rung invocations on a repeat-heavy step drop ≥50% with zero decision changes; every artifact-byte/decision invariant asserted green.
