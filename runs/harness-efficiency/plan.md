...
- **State-classification predicates (exact, fail-closed).** A stale heartbeat alone does not name a cause — the three status classifications are decided from four observables sampled together at detection time: `pid_alive` (liveness of the recorded `heartbeat.pid` via `os.kill(pid, 0)`); `hb_age_s` = `now_wallclock − heartbeat.wallclock_utc`; `clock_skew` = the primary **or** fallback detector fired on the last consecutive-heartbeat pair (real-time jump the monotonic clock did not see); `agent_output_age_s` = time since the running step's adapter child last appended to `events.jsonl`. The predicates are disjoint and evaluated in this order:
  1. **`host_suspended`** ⟺ `hb_age_s > interval + 30s` **and** `pid_alive` **and** `clock_skew` present. The gap is explained by a real-time jump; the interval is recorded and **credited** to the deadline (bounded by the cap).
  2. **`driver_orphaned`** ⟺ `hb_age_s > interval + 30s` **and not** `pid_alive`. The writer died, so no heartbeat was written across the gap; the interval is **not** credited (there was no suspend to credit) and recovery follows the orphaned-driver path.
...
- Unit (state predicates, one observable-tuple per case): `(pid_alive, stale, clock_skew)` → `host_suspended` + credited interval; `(pid_dead, stale)` → `driver_orphaned` + **no** credit; `(pid_alive, fresh-heartbeat, agent_output_age > agent_silence_s)` → `agent_silent` + no credit; each renders the matching classification in `status --json`.
