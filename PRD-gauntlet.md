...
**Status:** Draft v1.5 (v1.4 + judge-policy push/PR boundary: bound the harness's *autonomy* — no autonomous push/PR/merge in a run — while letting the operator's own directed sessions push feature branches and create/read PRs; §2.2, FR-9.8)
...
- **No automatic merging or pushing by the harness.** A `gauntlet run` commits to a working branch and never autonomously pushes, opens, or merges a PR — it drafts `runs/<slug>/PR.md` and stops; opening and merging the PR remain human actions. This bounds the *pipeline's* autonomy, not the human operator: the judge policy still permits a human-directed Claude Code session (this repo's bootstrap, or any non-gauntlet project) to push feature branches and create/read PRs. The latitude of an operator session vs. an in-run agent is set by the judge policy (FR-9.8).
...
- **FR-9.8** **PR creation.** When the final phase gate passes, Gauntlet drafts (does not open) a PR description into `runs/<slug>/PR.md`: PRD summary, per-phase commit list including fix rounds, links to run transcripts and final per-finding verdicts. **The harness never autonomously pushes, opens, or merges a PR**; opening the PR remains a human action (`gauntlet pr open`, a gh-CLI convenience that opens the drafted PR on a human's command, remains a v1.1 candidate). Enforcement is two-layered: (1) the pipeline issues no push/PR step, and (2) defense-in-depth, the judge policy denies `git push` and `gh pr create` whenever a pipeline step is active (the decide request carries a `step_id`), and denies force-push, remote-branch deletion, pushes to `main`/`master`, and `gh pr merge` in *every* context. Outside a run, the same judge policy permits a human-directed operator session to push feature branches and create/read PRs — the v1 non-goal bounds the harness's autonomy, not the operator (§2.2). (Earlier drafts stated "pushing and PR creation remain human actions in v1" without distinguishing the harness from the operator; v1.5 draws that line explicitly so the shared judge policy can serve both roles.)
